...
c.JupyterHub.bind_url = 'http://:' + (os.environ.get('HUB_CONTAINER_PORT') or '8000')
...
c.JupyterHub.load_roles = [
    {
        ...
        'description': 'Grants permission to cull idle notebooks and users',
        'scopes': [
            'list:users',
            'read:users:activity',
            'read:servers',
    ...
    {
        'name': 'admin-rights',
        'description': 'Grants full rights to all actions',
        'scopes': [
            'admin:users',
            'admin:servers',
            'admin:groups',
...
idle_culler_timeout = int(os.environ.get('IDLE_CULLER_TIMEOUT') or 3600)
c.JupyterHub.services = [
    {
...
c.DockerSpawner.image = os.environ['LAB_IMAGE']
c.DockerSpawner.network_name = os.environ['DOCKER_NETWORK_NAME']
# The port for single-user servers to listen on.
# Defaults to 0, which uses a randomly allocated port number each time.
# If set to a non-zero value, all Spawners will use the same port, 
# which only makes sense if each server is on a different address, e.g. in containers.
c.DockerSpawner.port = int(os.environ.get('LAB_CONTAINER_PORT') or 0)
...
# Bind mount volumes
mounts = []
for mount_pair in (os.environ.get('LAB_BIND_MOUNTS') or '').split(','):
    source, target = mount_pair.strip().split(':')
...
# Add `runtime` configuration to switch to nvidia container runtime for CUDA
c.DockerSpawner.extra_host_config = {}
container_runtime = os.environ.get('LAB_RUNTIME')
if container_runtime is not None:
    c.DockerSpawner.extra_host_config['runtime'] = container_runtime
...
c.Authenticator.allowed_users = {
    user.strip()
    for user in (os.environ.get('ALLOWED_USERS') or '').split(',')
}
